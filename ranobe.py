...
from Source.Core.Base.Formats.Ranobe import Branch
...
class Parser(RanobeParser):
	...
	def __ParseCover(self, soup: BeautifulSoup):
		...
		self._Title.add_cover(Cover(self._SystemObjects, self).set_link(f"https://{self._Manifest.site}" + soup.find("div", {"class": "poster"}).find("img")["src"]))
